$(function () {
    ...
    $("<br/>").appendTo('body');
    $("<canvas id='" + Constant.CANVAS_ID + "' width='600' height='400'></canvas>").appendTo('body');
...
var SnakeGame = {
    ...
    gameover: function () {
        ...
        var _sameItem_ = Constant.TargerArrayIsInArraySet(this.snake.head, this.snake.body);
        this.snake.body.pop();
    ...
    frameSubtitle: function (text) {
        ...
        this.ctx.fillText(text, this.canvas_width / 2 - 10 * text.length, this.canvas_height / 2);
        this.ctx.restore();
    },
...
var Snake = {
    ...
    moveAndEat: function (food, offsetX, offsetY) {
        ...
        if (food && _head[0] == food.X && _head[1] == food.Y) {
            ...
            } while (Constant.TargerArrayIsInArraySet([ food.X, food.Y ], _tmp_));
        } else {
    ...
    /**
     * 绘制Snake
     * @param ctx {Object} Canvas 2D Object
     */
    render: function (ctx) {
        // 绘制头
        ctx.save();
        ...
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.arc(0, 0, Constant.UNIT_LEN / 2, Math.PI / 6, 11 * Math.PI / 6, false);
        ctx.fill();
        ctx.closePath();
        ...
        ctx.arc(0, -Constant.UNIT_LEN / 2 / 2, Constant.UNIT_LEN / 10, 0, 2 * Math.PI, false);
        ctx.fill();
        ctx.closePath();
        ...
        for (var i = 0; i < iLen; i++) {
            ctx.beginPath();
            ctx.moveTo(this.body[i][0] + Constant.UNIT_LEN / 2, this.body[i][1] + Constant.UNIT_LEN / 2);
            ctx.arc(
                this.body[i][0] + Constant.UNIT_LEN / 2
                , this.body[i][1] + Constant.UNIT_LEN / 2
                , 4 * Constant.UNIT_LEN / 2 / 5
                , 0
                , 2 * Math.PI, false
            );
            ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(this.tail[0] + Constant.UNIT_LEN / 2, this.tail[1] + Constant.UNIT_LEN / 2);
        ctx.arc(
            this.tail[0] + Constant.UNIT_LEN / 2
            , this.tail[1] + Constant.UNIT_LEN / 2
            , 3 * Constant.UNIT_LEN / 2 / 5
            , 0
            , 2 * Math.PI, false
        );
        ctx.fill();
...
var Food = {
    ...
    render: function (ctx) {
        ...
        ctx.beginPath();
        ctx.arc(
            Food.X + Constant.UNIT_LEN / 2
            , Food.Y + Constant.UNIT_LEN / 2,
            3 * Constant.UNIT_LEN / 2 / 4
            , 0
            , Math.PI * 2, true
        );
        ctx.closePath();
